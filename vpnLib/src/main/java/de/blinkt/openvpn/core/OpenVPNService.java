...
public class OpenVPNService extends VpnService implements StateListener,
        Callback, ByteCountListener, IOpenVPNServiceInternal {

    public static final int NOTIFY_ID = 0x1221;
    ...
    public static final String NOTIFICATION_CHANNEL_NEWSTATUS_ID = "com.aman.vpnClient.VPN_STATE_NOTIFICATION";
    ...
    public void challengeResponse(String response) throws RemoteException {
        if (mManagement != null) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
                b64response = Base64.encodeToString(response.getBytes(StandardCharsets.UTF_8), Base64.DEFAULT);
            } else {
                b64response = Base64.encodeToString(response.getBytes(), Base64.DEFAULT);
            }
        ...
        switch (status) {
            case LEVEL_CONNECTED:
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    nbuilder.setColor(ContextCompat.getColor(this, R.color.success_text));
                }
                break;
            case LEVEL_CONNECTING_SERVER_REPLIED:
                nbuilder.setSmallIcon(R.drawable.ic_notification);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    nbuilder.setColor(ContextCompat.getColor(this, R.color.warning_text));
                }
                break;
            default:
                nbuilder.setSmallIcon(R.drawable.ic_notification);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    nbuilder.setColor(ContextCompat.getColor(this, R.color.error_text));
                }
                break;
        }

        if (mProfile != null) {
            nbuilder.setContentTitle(getString(R.string.notifcation_title_connect) + " to " + mProfile.mName);
